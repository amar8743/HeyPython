...
def get_data(orders_table_name):
    try:
        ...
        stocks_filtered.head(10).to_json("demoout4.json", orient="records", indent=4)

        stocks_to_db = stocks[['Symbol', 'Price']].reset_index().rename(columns={'Date': 'Dt'}).round(2)
        stocks_to_db.head(10).to_json("demoout5.json", orient="records", indent=4)
        
        stocks_to_db = stocks_to_db.astype({'Dt': str})
        stocks_to_db.head(10).to_json("demoout6.json", orient="records", indent=4)
