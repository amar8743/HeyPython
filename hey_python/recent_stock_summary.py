...
def getData():
    ...
    stocks.head(3).to_json("demoout1.json", orient="records", indent=4)

    stocks_to_exclude = stocks[stocks['Price']/stocks['Prev'] < .99]
    stocks_to_exclude.head(3).to_json("demoout2.json", orient="records", indent=4)
    ...
    stocks_filtered.head(3).to_json("demoout4.json", orient="records", indent=4)

    stocks_to_db = stocks[['Symbol', 'Price']].reset_index().rename(columns={'Date': 'Dt'}).round(2)
    stocks_to_db.head(3).to_json("demoout5.json", orient="records", indent=4)
